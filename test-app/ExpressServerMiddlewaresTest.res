...
module FileParseConfig: FileParseConfig = {
  let getFileAwaitFields: unit => array<fileAwaitField> = () => [{name: "fileInp", maxCount: 1}]
  let getDestPath: unit => string = () => "/uploads"
}
module FileConverter = ExpressFileHandlerConverter.Make(SessionHandler, FileParseConfig)
...
let routes = [
  ...
  Route(
    #get,
    "/set-session",
    OnlySessionHandler.handler(_ => {
      SessionRes(Redirect("/show-session", #302), [SetSessionValue("a", %raw(`12`))])
    }),
  ),
  Route(
    #get,
    "/delete-session",
    OnlySessionHandler.handler(_ => {
      SessionRes(Redirect("/show-session", #302), [DestroySession])
    }),
  ),
]
