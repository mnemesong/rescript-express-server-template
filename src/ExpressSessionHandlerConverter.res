...
module Make: Make = (OldHandler: Handler, SessionConfigurator: SessionConfigurator) => {
  ...
  let applyStepRes: (newReq, newRes) => oldRes = (newReq, newRes) => {
    ...
    let applySetSessionVal: (request, string, unknown) => unit = %raw(` 
    function(req, k, v) {
      req.session[k] = v;
    } 
    `)
    let applyDestroySession: request => unit = %raw(`
    function(req) {
      req.session.destroy();
    }
    `)
