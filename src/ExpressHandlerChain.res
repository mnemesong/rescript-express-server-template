...
module Make: Make = (
  ...
  let wrapReq = (r: (request, response)) => OldHandler.wrapReq(r)->Converter.wrapStepReq

  let applyRes: (hReq, hRes) => unit = (hReq, hRes) =>
  ...
  let primalReq: hReq => (request, response) = hReq =>
    Converter.getOldReq(hReq)->OldHandler.primalReq

  let middlewares = OldHandler.middlewares->Array.concat(Converter.middlewares)
