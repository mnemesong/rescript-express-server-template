...
module Make: T = (
    ...
    let wrapStepReq = (oldReq: oldReq): newReq => {
        ...
        `)
        let maybeUser = parse(req)
            -> Option.flatMap((ucad) => UserManager.checkAuthData(ucad))
    ...
    let applyStepRes = (newReq: newReq, newRes: newRes): oldRes => {
        ...
        let applyLoginAuthData: (request, UserManager.userCheckAuthData) => unit = 
        %raw(` function(req, u) {
            req.session.userCheckAuthData = u;
        } `)
        let applyLogout: (request) => unit = 
        %raw(`function(req) {
            delete req.session.userCheckAuthData;
        }`)
        Array.forEach(authSessEffects, (e) => switch e {
            | Login(u) => {
